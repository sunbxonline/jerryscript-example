...
static jerry_value_t
image_constructor_handler(const jerry_value_t func_value, /**< function object */
                 const jerry_value_t this_value, /**< this arg */
                 const jerry_value_t *args_p, /**< function arguments */
                 const jerry_length_t args_cnt) /**< number of function arguments */
{
  printf("image_constructor_handler\n");
  (void)func_value;
  (void)args_p;
  (void)args_cnt;
  struct ImageView *image = (struct ImageView*)malloc(sizeof(struct ImageView));
  image_initiator((struct View*)image);
  jerry_set_object_native_pointer(this_value, image, NULL);
  return jerry_create_undefined();
}
      ...
      char **argv)
{
  ...
  jerry_value_t image_constructor = jerry_create_external_function(image_constructor_handler);
  jerry_release_value(jerry_set_property (image_constructor,
        jerry_create_string ((const jerry_char_t *) "prototype"), 
        g_image_prototype_object));

  jerry_value_t glob_obj_val = jerry_get_global_object ();
  jerry_release_value (jerry_set_property (glob_obj_val , 
        jerry_create_string ((const jerry_char_t *) "Image"), 
        image_constructor));

  const jerry_char_t script[] = " \
    var myview = new Image(); \
    myview.x = 35; \
    print(myview.x); \
    myview.src = \"segment\"; \
    print(myview.src); \
